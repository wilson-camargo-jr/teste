...
Primeiramente, vamos ajustar o R Studio para trabalhar em nossa recém criada área de trabalho, utilizando a aba “Files” para o posicionamento, conforme a figura abaixo:
...
Em seguida vamos configurar essa pasta como o Diretório de Trabalho, acionando o ícone “More” e selecionando a opção “Set As Working Directory”, conforme figura abaixo:
...
Em seguida insira os comentários abaixo:
...
As fases mais longas e trabalhosas das atividades de um cientista de dados são as fases de obtenção e preparação dos dados.

Na primeira, é necessário localizar os dados e montar todos os processos administrativos e operacionais para obter, armazenar e disponibilizar os mesmos para as etapas seguintes.

Na fase de preparação é preciso descobrir falhas ocorridas na obtenção dos dados, valores ausentes, tratamento de exceções, valores com erros de digitação,  dados repetidos, etc., e a melhor forma de descobrir o que fazer é examinando os dados.

Nosso dataset (aqui simplificamos a fase de obtenção) está em 2 arquivos que obtivemos por download, mas como nosso propósito é mostrar como trabalhamos com dados armazenados em banco de dados, então nossa primeira tarefa aqui será jogar os dados para tabelas no banco de dados. Isso é feito de forma muito simples na própria console do R Studio, com os comandos abaixo.
...
Perceba que já temos um objeto R do tipo ore.frame sem termos carregado nenhum dado para a memória local.
...
As colunas da tabela que criamos na carga dos dados.
...
Número de linhas e coluna do nosso dataset. 
...
Vamos dar uma olhada nos dados do nosso dataset ?

head(train)

> head(train)    
   Loan_ID Gender Married Dependents    Education
1 LP001996   Male      No          0     Graduate 
2 LP001998   Male     Yes          2 Not Graduate 
3 LP002002 Female      No          0     Graduate 
4 LP002004   Male      No          0 Not Graduate 
5 LP002006 Female      No          0     Graduate 
6 LP002008   Male     Yes          2     
       Graduate   Self_Employed ApplicantIncome CoapplicantIncome 
1            No           20233                 0 
2            No            7667                 0 
3            No            2917                 0 
4            No            2927              2405 
...
Esses são os 6 primeiros registros de nosso dataset, e ao observar com atenção, já vemos um problema. Perceba que nos registros 2 e 6 da coluna “Credit_History” encontramos um NA, ou seja, não existe a informação.
...
> summary(train)
    Loan_ID       Gender    Married    Dependents        Education    LP001002:  1   Male  :489   Yes :398   0   :345   Graduate    :480    LP001003:  1   Female:112   No  :213   1   :102   Not Graduate:134    LP001005:  1   NA's  : 13   NA's:  3   2   :101                       LP001006:  1                           3+  : 51                       LP001008:  1                           NA's: 15                       LP001011:  1                                                          (Other) :608                                                          Self_Employed ApplicantIncome CoapplicantIncome   LoanAmount     
...
Min.   : 12      Min.   :0.0000   Semiurban:233   Y:422        
1st Qu.:360      1st Qu.:1.0000   Urban    :202   N:192        
Median :360      Median :1.0000   Rural    :179                
...
Vemos que existem diversas colunas com ausencia de informação em seus registros, por exemplo, na coluna Gender, temos 489 registros com valores “Male”, 112  com “Female” e 13 com informações ausentes.

Temos então que decidir o que fazer com os valores ausentes em cada coluna. Poderíamos simplesmente excluir as linhas que possuem colunas NA, ou preencher os ausentes com algum valor que seja mais apropriado, por exemplo,  a média aritmética das observações da coluna, o valor mais frequente, etc. 

A análise dever ser feita caso a caso, lembrando que sempre em qualquer das hipóteses de remoção ou substituição existe a possibilidade de estarmos inserindo algum componente de erro em nosso modelo.

a) Lidando com valores não disponíveis (NA)

Vamos carregar nossos datasets “train” e “test” em variáveis locais para a realização dos devidos ajustes. Vamos inserir o seguinte código no editor e executar os comandos:
...
Vemos rever quais colunas apresentam valores nulos em seus registros.  Execute o seguinte comando na console do R studio.
